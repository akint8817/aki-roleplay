...
function novelanceRingBuildings(cx: number, cy: number, rInner: number, rOuter: number, color: string, angStepDeg: number, radialBands: number, seed: number, ruined?: boolean): string {
  let s = seed;
...
function novelanceScatterBuildings(poly: [number,number][], color: string, count: number, seed: number, ruined?: boolean): string {
  let s = seed;
  const rand = () => { s = (s*1103515245+12345) & 0x7fffffff; return (s % 1000)/1000; };
