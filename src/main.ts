...
// Bascule le mode édition de la page Halcyon (bouton "Modifier", visible
// seulement connecté) — voir renderHalcyonPage.
function toggleHalcyonEditMode(): void {
  halcyonEditMode = !halcyonEditMode;
  render();
}

// Dirigeant actuel de Halcyon, affiché en haut de la page Halcyon — un seul
// document partagé, modifiable en place via le bouton "Modifier".
...
// même position s'affichent côte à côte, comme des branches. "desc" suit les
// mêmes règles de mise en forme que le reste du site (voir renderRichBody).
...
// Carte façon grille de champions (League of Legends) : portrait, nom et
...
function initPersonnageEntryRail(): void {
  ...
  if(hero){
    const img = hero.querySelector<HTMLElement>('.op-hero-badge');
