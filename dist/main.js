...
let halcyonTierDocsCache = [];
let orgTierEditId = null;
...
function initFirestoreSync() {
    ...
    db.collection('halcyonInfo').doc('main').onSnapshot((doc) => {
        const data = doc.data() || {};
    ...
    db.collection('halcyonTierOverrides').onSnapshot((snap) => {
        const list = [];
        snap.forEach((doc) => {
            const data = doc.data();
        ...
        render();
        restoreDraftFormState(draft);
    }, (err) => console.error('Firestore (halcyonTierOverrides) :', err));
...
function handleNewSquadProjectImage(squadId, input) {
    ...
    const reader = new FileReader();
    reader.onload = () => {
        spjImageDraft[squadId] = reader.result;
        input.value = '';
        refreshNewSquadProjectImagePreview(squadId);
    };
    reader.readAsDataURL(file);
}
function removeNewSquadProjectImage(squadId) {
    delete spjImageDraft[squadId];
    refreshNewSquadProjectImagePreview(squadId);
}
function newSquadProjectImagePreviewHtml(squadId) {
...
function handleSquadProjectImage(id, input) {
    ...
    const reader = new FileReader();
    reader.onload = () => {
        db.collection('halcyonSquadProjects').doc(id).update({ image: reader.result });
        input.value = '';
    };
    reader.readAsDataURL(file);
}
function removeSquadProjectImage(id) {
    const db = getFirestoreDb();
    if (!db) return;
...
function addOrgTier(besideId) {
    const labelEl = document.getElementById('otLabel');
    const descEl = document.getElementById('otDesc');
...
function toggleOrgTierEditMode(id) {
    orgTierEditId = orgTierEditId === id ? null : id;
    render();
}
function saveOrgTier(id) {
    const labelEl = document.getElementById('otEditLabel-' + id);
    const descEl = document.getElementById('otEditDesc-' + id);
...
function saveSquadDossier(id) {
    const nameEl = document.getElementById('sqdName');
...
function addSquadProject(squadId) {
    ...
    const typeEl = document.getElementById('spjType-' + squadId);
    const noteEl = document.getElementById('spjNote-' + squadId);
...
function rosterBubbleHtml(rosterId, e) {
    const c = CATS[e.cat];
    return `
...
function rosterStepX(rosterId, isMobile) {
...
function layoutRosterStage(rosterId) {
...
function openRosterBubble(rosterId, index) {
    const state = getRosterState(rosterId);
    if (!state)
        return;
...
function closeRosterBubble(rosterId) {
    const bubble = document.getElementById('rosterBubble-' + rosterId);
    if (bubble)
...
function onRosterCardHoverStart(rosterId, index) {
    const state = getRosterState(rosterId);
    if (!state || index !== state.selected)
        return;
...
function onRosterCardHoverEnd(rosterId, _index) {
    if (rosterHoverTimer !== null) {
...
function openRosterZoom(rosterId, index) {
    const state = getRosterState(rosterId);
    if (!state)
        return;
    const e = state.list[index];
...
function onRosterStageClick(rosterId, ev) {
    if (rosterDrag.moved) {
...
function onRosterStageWheel(rosterId, ev) {
    const state = getRosterState(rosterId);
    if (!state || !state.list.length)
        return;
...
function onRosterStageMouseMove(rosterId, ev) {
    if (rosterDrag.active)
        return;
    const stage = document.getElementById('rosterStage-' + rosterId);
    const active = stage ? stage.querySelector('.roster-3d-card.is-active') : null;
...
function onRosterStageMouseLeave(rosterId) {
    if (rosterDrag.active)
        return;
    const stage = document.getElementById('rosterStage-' + rosterId);
    const active = stage ? stage.querySelector('.roster-3d-card.is-active') : null;
...
function onRosterStageMouseDown(rosterId, ev) {
    const stage = document.getElementById('rosterStage-' + rosterId);
    const active = stage ? stage.querySelector('.roster-3d-card.is-active') : null;
    if (!active)
        return;
...
function initRosterStage(rosterId) {
    layoutRosterStage(rosterId);
    const stage = document.getElementById('rosterStage-' + rosterId);
    if (stage) {
...
function selectRosterCard(rosterId, index) {
    const state = getRosterState(rosterId);
    if (!state)
        return;
...
function scrollRoster(rosterId, dir) {
    const state = getRosterState(rosterId);
    if (!state)
        return;
    const next = state.selected + dir;
...
function musicBarHtml(ownerId, music) {
    if (!music) return '';
